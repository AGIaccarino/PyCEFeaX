...
# %%
#Loading the dataset as pfx dataframe
cata=pd.read_csv("./AQ2009_catalogue.csv",index_col=0)
cata["source_Mw"]=1.066*cata["source_magnitude_DD"]-0.164 #ML to Mw Gasperini et al 2013
cata_pfx=pfx.make_df(cata["source_origin_time_DD"], cata["source_latitude_deg_DD"], cata["source_longitude_deg_DD"], cata["source_depth_km_DD"],cata["source_Mw"])
...
# === Loads TIFF RGB for L'Aquila area===
...
# === Plots the map with earthquakes===
...
sc = ax.scatter(
    ...
    cmap='viridis_r',
    s=cata_pfx['source_magnitude'].iloc[sorted_idx]*5+10,
    edgecolor='k',
...
# %%
pfx.plot_allfeatures(data, datetime(2009,4, 6), datetime(2009, 10, 6))
...
#data selection
t_AQ = datetime(2009, 4, 6) + timedelta(hours=11.5)
t_start=t_AQ
t_end = t_AQ + timedelta(hours=24)
